        ...
        <QueryClientProvider client={queryClient}>
          <BrowserRouter>
            <div className="min-h-screen transition-colors duration-300" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
              <Header onLocalFileLoaded={setLocalFile} />
                      ...
                      {localFile.data.pl?.map((pl: any) => (
                        <div key={pl.cd} className="border border-gray-100 dark:border-slate-800 rounded-lg p-3 bg-gray-50/50 dark:bg-slate-800/30">
                          ...
                          <div className="space-y-2">
                            {pl.e?.map((e: any) => (
                              <div key={e.cd} className="bg-white dark:bg-slate-900 p-3 rounded border border-gray-200 dark:border-slate-700 shadow-sm">
                                <div className="flex justify-between items-start">
                                  <div>
                                    <div className="font-bold text-gray-900 dark:text-gray-100">{e.nm}</div>
